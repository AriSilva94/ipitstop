...
type Client = { id: string; name: string; phone: string | null; notes: string | null }
...
export default function EditClientForm({ shopSlug, client }: { shopSlug: string; client: Client }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await updateClientAction(shopSlug, client.id, formData)
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-slate-800 border border-slate-700 rounded-2xl p-6 space-y-4">
      <div>
        ...
        <input name="name" required type="text" defaultValue={client.name}
      ...
      <div className="flex flex-col sm:flex-row gap-3 pt-1">
        ...
        <a href={`/${shopSlug}/app/clients/${client.id}`}
