...
export default function NewCarPage() {
  const { shopSlug, clientId } = useParams<{ shopSlug: string; clientId: string }>()
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await createCarAction(shopSlug, clientId, formData)
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-lg w-full">
      ...
      <form onSubmit={handleSubmit} className="bg-slate-800 border border-slate-700 rounded-2xl p-6 space-y-4">
        <div>
          ...
          <input name="plate" required type="text" placeholder="ABC-1234"
            className={`${inputClass} font-mono tracking-widest uppercase`} />
        </div>
        <div>
          <label className={labelClass}>Modelo *</label>
          <input name="model" required type="text" placeholder="Gol 1.0"
            className={inputClass} />
        </div>
        <div>
          <label className={labelClass}>Ano</label>
          <input name="year" type="number" placeholder="2020" min={1900} max={new Date().getFullYear() + 1}
            className={inputClass} />
        </div>
        <div>
          <label className={labelClass}>Observações</label>
          <textarea name="notes" rows={3} placeholder="Cor, acessórios, observações..."
            className={inputClass} />
        </div>
